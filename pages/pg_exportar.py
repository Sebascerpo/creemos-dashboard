...
def render(datos: dict):
            ...
            [
                {
                    ...
                    "Municipios": v.get("n_municipios", 0),
                }
                for k, v in sorted(
                    mmv["candidatos"].items(),
                    key=lambda x: x[1]["votos_total"],
                    reverse=True,
                )
            ]
        )
    ...
    df_munis = pd.DataFrame(
        [
            ...
            for k, v in sorted(
                mmv["municipios"].items(),
                key=lambda x: x[1]["votos_validos"],
                reverse=True,
            )
        ]
    )
    ...
    df_deptos = pd.DataFrame(
        [
            ...
            for cod, d in sorted(
                mmv["deptos"].items(), key=lambda x: x[1]["votos_validos"], reverse=True
            )
        ]
    )
    st.download_button(
