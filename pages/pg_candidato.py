...
from pages.shared import (
    fmt, pct, kpi, section, badge, plotly_defaults,
    nombre_partido, nombre_municipio_str, nombre_depto,
)
...
def render(
    ...
    if solo_antioquia:
        ...
        por_muni    = {k: v for k, v in cd["por_municipio"].items()
                       if k.startswith(COD_ANTIOQUIA + "_")}
        por_puesto  = {k: v for k, v in cd["por_puesto"].items()
                       if k.startswith(COD_ANTIOQUIA + "_")}
        por_mesa    = {k: v for k, v in cd["por_mesa"].items()
                       if k.startswith(COD_ANTIOQUIA + "_")}
    ...
    circ_ref = (expected_circ or cd.get("circunscripcion") or (meta or {}).get("circunscripcion", "")).strip()
    ...
    if solo_antioquia:
        total_ref = sum(v for k, v in totales_muni.items() if k.startswith(COD_ANTIOQUIA + "_"))
    ...
    with c1:
        kpi("Total votos", fmt(votos_total),
            nombre_partido(cd["cod_partido"], partidos)[:40], color)
    with c2:
        kpi("% sobre total", pct(votos_total, total_ref),
            total_ref_label, color)
    with c3:
    ...
    df_top30 = pd.DataFrame([{
        ...
        "% municipio": pct(v, mmv["municipios"].get(k, {}).get("votos_validos", 1)),
    ...
    with col_puesto:
        if puestos_opc:
            sel_puesto = puestos_opc[st.selectbox(
                "Puesto de votacion", list(puestos_opc.keys()),
    ...
    with col_mesa:
        if mesas_opc:
            sel_mesa = mesas_opc[st.selectbox(
                "Mesa", list(mesas_opc.keys()), key=f"dd_mesa_{cand_key}")]
        else:
            st.info("Sin mesas")
    ...
    with r1:
        ...
        kpi("Votos en municipio", fmt(votos_muni),
            f"{muni_nom} · {pct(votos_muni, total_muni)} del total del municipio ({fmt(total_muni)} válidos)", color)
    with r2:
        kpi("Votos en puesto", fmt(votos_puesto),
            f"{pct(votos_puesto, total_puesto)} del total del puesto ({fmt(total_puesto)} válidos)", color)
    with r3:
        num_mesa = sel_mesa.split("_")[4] if sel_mesa else ""
        kpi("Votos en mesa", fmt(votos_mesa),
            f"Mesa #{num_mesa} · {pct(votos_mesa, total_mesa)} del total de la mesa ({fmt(total_mesa)} válidos)", color)
