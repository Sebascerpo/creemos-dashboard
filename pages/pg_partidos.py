...
from pages.shared import (
    ...
    nombre_partido, nombre_depto, nombre_municipio_str,
)
...
def _build_partidos_agg(mmv: dict) -> tuple[dict, dict]:
    ...
    for cod, d in partidos_por_circ.get("0", {}).items():
        ...
        part_senado[cod] = {
            ...
            "por_mesa": dict(d.get("por_mesa_validos_total", {})),
        }
    ...
    for cod, d in partidos_por_circ.get("1", {}).items():
        ...
        part_camara[cod] = {
            ...
            "por_municipio": _filter_prefix(d.get("por_municipio_validos_total", {}), pref),
            "por_puesto": _filter_prefix(d.get("por_puesto_validos_total", {}), pref),
            "por_mesa": _filter_prefix(d.get("por_mesa_validos_total", {}), pref),
        }
...
def _render_drilldown_partido(
    ...
    if fixed_dep:
        ...
        with col_dep:
            sel_dep = deptos_opc[st.selectbox(
                "Departamento",
...
def render(datos: dict):
    ...
    with tab_sen:
        ...
        if not part_senado:
            st.info("No hay datos de partidos para Senado (corporación 001, circunscripción 0).")
        else:
            rows_s = [{
                "Codigo": cod,
            ...
            } for cod, d in sorted(part_senado.items(), key=lambda x: x[1]["votos_total"], reverse=True)]
            ...
            opc_s = {
                ...
                for c, d in sorted(part_senado.items(), key=lambda x: x[1]["votos_total"], reverse=True)
            }
            ...
            with c1:
                df_ds = pd.DataFrame([{
                    ...
                    "Cod": d,
                    "Votos": v,
                } for d, v in sorted(pd_s["por_depto"].items(), key=lambda x: x[1], reverse=True)])
                if not df_ds.empty:
                    fig_ds = px.bar(
                        df_ds,
                        x="Departamento",
                    ...
                    fig_ds.update_layout(coloraxis_showscale=False, height=300, xaxis_tickangle=-35)
                    st.plotly_chart(plotly_defaults(fig_ds), use_container_width=True)
            with c2:
                top_ms = sorted(pd_s["por_municipio"].items(), key=lambda x: x[1], reverse=True)[:20]
                df_ms = pd.DataFrame([{
            ...
            _render_drilldown_partido(
                ...
                totales_mesa=sen_totales_mesa,
            )

    with tab_cam:
        section("PARTIDOS CAMARA - ANTIOQUIA", "how_to_vote")
        if not part_camara:
            st.info("No hay datos de partidos para Cámara (corporación 002, circunscripción 1).")
        else:
            rows_c = [{
                "Codigo": cod,
                "Partido": nombre_partido(cod, partidos),
                ...
                "Municipios": len(d["por_municipio"]),
            } for cod, d in sorted(part_camara.items(), key=lambda x: x[1]["votos_total"], reverse=True)]
            ...
            opc_c = {
                f"{nombre_partido(c, partidos)} ({fmt(d['votos_total'])} votos)": c
                for c, d in sorted(part_camara.items(), key=lambda x: x[1]["votos_total"], reverse=True)
            }
            sel_c = st.selectbox("Partido", list(opc_c.keys()), key="sel_cam")
            pd_c = part_camara[opc_c[sel_c]]

            top_mc = sorted(pd_c["por_municipio"].items(), key=lambda x: x[1], reverse=True)[:20]
            df_mc = pd.DataFrame([{
                "Municipio": nombre_municipio_str(k, divipol),
                "Clave": k,
                "Votos": v,
