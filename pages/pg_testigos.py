...
from pages.shared import section
...
def render(datos: dict):
    ...
    with st.expander("Agregar reporte de testigo", expanded=True):
        ...
        with col1:
            t_depto  = st.text_input("Código Depto (2 dígitos)",     max_chars=2, key="t_depto")
            t_muni   = st.text_input("Código Municipio (3 dígitos)", max_chars=3, key="t_muni")
        with col2:
        ...
        with col3:
            t_votos_juliana = st.number_input("Votos JULIANA (Senado)",       min_value=0, key="t_vj")
            t_votos_german  = st.number_input("Votos GERMAN DARIO (Cámara)", min_value=0, key="t_vg")
            t_testigo       = st.text_input("Nombre testigo", key="t_nombre")
            t_obs           = st.text_area("Observaciones",   key="t_obs", height=68)

        if st.button("Registrar reporte", use_container_width=True,
            ...
            if t_depto and t_muni and t_mesa:
                ...
                if mmv:
                    ...
                    if cj: votos_oficial_j = cj["por_municipio"].get(muni_key)
                    if cg: votos_oficial_g = cg["por_municipio"].get(muni_key)
    ...
    if st.session_state.testigos:
        ...
        rows = [{
            ...
            "Juliana (oficial)": t["oficial_juliana"] if t["oficial_juliana"] is not None else "—",
            "Estado Juliana":    semaforo(t["votos_juliana"], t["oficial_juliana"]),
            "German (testigo)":  t["votos_german"],
            "German (oficial)":  t["oficial_german"] if t["oficial_german"] is not None else "—",
        ...
        with ca:
            st.download_button(
                ...
                "testigos.csv", "text/csv",
                use_container_width=True,
                icon=":material/download:",
            )
        with cb:
            if st.button("Limpiar reportes", use_container_width=True,
